...
class ServiceAccount(models.Model):
    ...
    def save(self, *args, **kwargs):
        self.config = self.create_qwc_file(
            app_name=self.name, url=self.app_url, username=self.qbid
        )
...
class TicketQueue(models.Model):
    ...
    status = models.CharField(
        max_length=5, choices=TicketStatus.choices, default=TicketStatus.CREATED
    )
    method = models.CharField(
        choices=TicketMethod.choices, max_length=5, default=TicketMethod.GET
    )
...
class BaseObjectManager(models.Manager):
    ...
    def get_next_txn(self, ticket, *args, **kwargs):
        ...
        return (
            self.filter(batch_ticket=self.ticket)
            .exclude(batch_status=BaseObjectMixin.BatchStatus.BATCHED)
            .first()
        )
